...
uint8_t local_mac[7];
uint8_t local_ip[5];
uint8_t dest_ip[16]={0};
uint8_t broad_mac[7]={0xff,0xff,0xff,0xff,0xff,0xff,0x00};

uint8_t  destmac[7]={0x01,0x01,0x04,0x01,0x01,0x02,0x00};//01-01-04-01-01-02
uint8_t localmac[7]={0x01,0x01,0x04,0x03,0x01,0x01,0x00};//01-01-04-03-01-01

uint8_t sendnid[NIDLEN] = {'d','2','s','u','b','1',0,0,'d','2','r','m',0,0,0,0};
...
struct _Ether_CoLoR_data
{
	...
	uint8_t options_dynamic;//可变首部选项
...
static int CoLoR_ParseData(const Ether_CoLoR_data *pkg)
{
	...
	*(tempData.data+data_len) = '\0';//不清楚为什么唯独data后需要特别腾出1Byte作为结束符否则会越界读取
	...
	sprintf(temp,"GOT%s",tempData.data);//When this code file is moved to CoLoR, change "wangzhaoxu" to a char* witch contains the SID.
...
int CoLoR_Sendpkg ( char * destmac,char * localmac,char * ip,char * dest )
{
	int i;
	Ether_CoLoR_get pkg;
	...
	memcpy(pkg.nid, sendnid, NIDLEN);//NID
	...
	while(1)
	{
		while(1)
		{
			...
			if(strncmp(message,"GET",3) == 0)
			{
				...
				if ( GetLocalMac ( PhysicalPort,mac,ip ) ==-1 )
				{
					//关闭线程
					exit(0);
					free(fd);
				...
				int i=0;
				{
					...
					CoLoR_Sendpkg ( destmac,localmac,ip,dest );
				}
